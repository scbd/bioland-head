...
| Multi-tenant isolation | A request resolves to exactly its Site, or fails closed | siteCode derived from host on the server; configured redirect Hosts resolve through the cached reverse index; other nonempty hosts outside `*.{baseHost}` and existing localhost/loopback shapes (including bracketed IPv6 literals) fail closed with 400 before query/cookie fallback. Existing explicit-site/event-context shortcuts and middleware API/static exemptions remain; missing Site config raises 404 |
