...
| Multi-tenant isolation | A request resolves to exactly its Site, or fails closed, **provided the edge strips or normalises `x-forwarded-host`** | siteCode derived from host on the server; configured redirect Hosts resolve through the cached reverse index; other nonempty hosts outside `*.{baseHost}` and the loopback shapes (`localhost`, `127.0.0.1`, `::1`, `[::1]`, `[::]`) fail closed with 400 before query/cookie fallback, and a Host that normalises to nothing fails closed too. Because `x-forwarded-host` is read ahead of `Host`, the property holds only where a trusted edge (CloudFront/ALB/ingress) overwrites any client-supplied value - an external rollout prerequisite tracked as AADR 0002, not enforced by this origin. Existing explicit-site/event-context shortcuts and middleware API/static exemptions remain; missing Site config raises 404 |
