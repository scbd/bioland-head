...
<script setup>
    ...
    const { data:count, status:countStatus, error:countError } =  await useLazyFetch(`/api/list/geobon/count`, {  method: 'GET',query, getCachedData, key:'geobon-0-widget' });

    const index = computed(()=> randomArrayIndexTimeBased(Number(count.value)));

    const { data:record, status, error} =  await useLazyFetch(`/api/list/geobon/${index.value}`, {  method: 'GET',query, getCachedData, key:'geobon-1-widget' });
