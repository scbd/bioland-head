...
import { useSiteStore } from "~/stores/site";
    ...
    async function setup() {
        ...
        const { data:nationalReportSixUrl } = hasCountry? await useFetch(`/api/nr6/${hasCountry}`, {   credentials: 'include'   }) : ''

        const menu = ref({ title: t('National Report'), href: nationalReportSixUrl, class: ['main-nav-sub-heading', 'arrow'] });


        makeChildren(menu, t)
        return { t,  nationalReportSixUrl, menu } //nationalReportSixUrl, menu,
    }
