...
/**
 * Documented fallback base for gaiaApi-relative thesaurus domain endpoints.
 * Used by `getApiUrl` only when `runtimeConfig.public.gaiaApi` is unavailable
 * (e.g. outside a request/Nitro context), and by the module-scope derived
 * exports below (`thesaurusApiUrls`), which cannot read runtime config at
 * import time. It is never used to build a URL at request time otherwise.
 */
...
export interface DataSourceConfig {
  source: 'api' | 'static';
  /**
   * Either a path relative to the gaiaApi thesaurus domains base (e.g.
   * `regions/terms`), or a fully-qualified URL for an external API (e.g.
   * the UN SDG API) that is unrelated to `gaiaApi`. Omitted for static and
   * composite domains.
   */
  path?: string;
  searchable?: boolean;
...
export const dataSourceConfigs: Record<string, DataSourceConfig> = {
  ...
  orgTypes:        { source: 'api', path: 'Organization%20Types/terms', searchable: true, sanitizer: { type: 'Organization', filter: (item) => !GOV_TYPE_IDS.has(item.identifier!) }},
  govTypes:        { source: 'api', path: 'Organization%20Types/terms', searchable: true, sanitizer: { type: 'GovernmentOrganization', filter: (item) => GOV_TYPE_IDS.has(item.identifier!) }},
  aichis:          { source: 'api', path: 'AICHI-TARGETS/terms', searchable: true, sanitizer: { type: 'Project', transform: (item) => {
  ...
  subjects:        { source: 'api', path: 'CBD-SUBJECTS/terms', searchable: true, sanitizer: { type: 'Project' }},
  jurisdictions:   { source: 'api', path: '50AC1489-92B8-4D99-965A-AAE97A80F38E/terms', searchable: true, sanitizer: { type: 'Thing' }},
  geoScopes:       { source: 'api', path: '4D4413D8-36F9-4CD2-8CC1-4F3C866DDE5A/terms', searchable: true, sanitizer: { type: 'Thing' }},
  projectStatuses: { source: 'api', path: '4E7731C7-791E-46E9-A579-7272AF261FED/terms', searchable: true, sanitizer: { type: 'Project' }},
  documentTypes:   { source: 'api', path: 'A762DF7E-B8D1-40D6-9DAC-D25E48C65528/terms', searchable: true, sanitizer: { type: 'CreativeWork', filter: (item) => DOC_TYPE_IDS.has(item.identifier!) }},
  gbfTargets:      { source: 'api', path: 'GBF-TARGETS/terms', searchable: true, sanitizer: { type: 'Project', transform: (item) => {
  ...
  gbfGoals:        { source: 'api', path: 'GBF-GOALS/terms', searchable: true, sanitizer: { type: 'Thing' }},
  eventStatuses:   { source: 'api', path: 'NCHM-EVENT-STATUS/terms', searchable: true, sanitizer: { type: 'Thing' }},
  bchSubjects:     { source: 'api', path: '043C7F0D-2226-4E54-A56F-EE0B74CCC984/terms', searchable: true, sanitizer: { type: 'Thing' }},

  // External APIs (absolute URLs, unrelated to gaiaApi — not joined with the thesaurus domains base)
  sdgs: { source: 'api', path: 'https://unstats.un.org/SDGAPI/v1/sdg/Goal/List?includechildren=false', searchable: true, sanitizer: { type: 'Project', transform: (item) => {
  ...
  sdts: { source: 'api', path: 'https://unstats.un.org/SDGAPI/v1/sdg/Target/List?includechildren=false', searchable: true, sanitizer: { type: 'Project', transform: (item) => {
...
/**
 * Join the documented default base with a domain's path. Used only to build the
 * module-scope `thesaurusApiUrls` snapshot below, which is computed once at import
 * time and therefore cannot read runtime config — see `getApiUrl` for the per-request
 * resolver that non-default environments should use instead.
 */
const buildDefaultUrl = (path: string) => (isAbsoluteUrl(path) ? path : `${API_BASE}/${path}`);
...
export const apiDomains = Object.entries(dataSourceConfigs).filter(([, c]) => c.source === 'api' && c.path).map(([k]) => k);
export const thesaurusApiUrls = Object.fromEntries(apiDomains.map(k => [k, buildDefaultUrl(dataSourceConfigs[k].path!)]));

export const isValidDomain = (domain: string) => dataSources.includes(domain);

/**
 * Resolve the fully-qualified API URL for a thesaurus domain, joining the current
 * `runtimeConfig.public.gaiaApi` with the domain's configured path.
 *
 * This is deliberately resolved per call rather than as a module-scope constant:
 * `useRuntimeConfig()` needs a request/Nitro context that does not exist at import
 * time, so calling it at module scope would either throw or capture a stale value.
 * Resolving inside this function means every call sees the current runtime config.
 *
 * Falls back to the documented `API_BASE` default when runtime config is unavailable
 * (e.g. called outside a request, or from a plain unit test with no Nitro context) —
 * that fallback reproduces today's hardcoded production URL exactly.
 *
 * External, non-gaiaApi domains (e.g. the UN SDG API) store an absolute URL as their
 * path and are returned unchanged.
 */
export const getApiUrl = (domain: string): string | undefined => {
  const path = dataSourceConfigs[domain]?.path;
  if (!path) return undefined;
  if (isAbsoluteUrl(path)) return path;

  let base = API_BASE;
  try {
    const gaiaApi = useRuntimeConfig()?.public?.gaiaApi;
    if (gaiaApi) base = `${gaiaApi}/v2013/thesaurus/domains`;
  } catch {
    // No request/Nitro context available (e.g. module import, plain unit test) —
    // use the documented default base.
  }
  return `${base}/${path}`;
};
